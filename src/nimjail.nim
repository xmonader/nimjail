...
when isMainModule:
  ...
  var cfg =  ChildConfig(argc:1, hostname:newHostName(), uid:0, mountDir:"/tmp/myrootfs")
  ...
  proc childAction(cnf: ptr ChildConfig) :  cint  =  
    ...
    # echo execCmdEx("busybox hostname")[0]
    let exe = findExe("busybox")
    echo "execve ls on /  ..."
    var args = allocCStringArray(@[exe, "ls", "/usr/"])
